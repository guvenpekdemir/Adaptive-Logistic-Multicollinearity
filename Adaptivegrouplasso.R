...
alasso <- cv.glmnet(x = x_train_std, y = y_train_std, weigths = best_ridge_weights,
                    family = "multinomial", nfolds = 10, type.multinomial="grouped",
                    intercept=FALSE, type.measure ="class", penalty.factor=rep(1,7),
                    alpha = 1)
...
set.seed(1000)
std_lasso_logit <- glmnet(x_train_std, y_train_std,family="multinomial",weigths = best_ridge_weights,
                          type.measure = "class", penalty.factor=rep(1,7),
                          alpha=1, lambda = alambdalasso)
...
summary(multiclass.roc(y_test_std, as.numeric(SRL_pred_test)))
